...
import { Activity, AlertTriangle, Thermometer, Gauge, Wind, Zap, Droplets } from 'lucide-react'
...
export default function DevicePage({ updateAppState }: Props) {
  ...
  // 加载设备列表
  useEffect(() => {
    api.get<any[]>('/devices').then(data => {
      const deviceList = (data as any[]).map(d => ({
      ...
      if (data.type === 'metrics:update' && data.device_id) {
        setDevices(prev => prev.map(d =>
          d.device_id === data.device_id
            ? { ...d, metrics: { ...d.metrics, ...data.metrics }, updated_at: new Date().toISOString() }
            : d
        ))
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            className="rounded-lg bg-industrial-card border border-industrial-border px-3 py-1.5 text-sm text-industrial-text outline-none focus:border-industrial-primary"
          >
            {devices.map(d => (
